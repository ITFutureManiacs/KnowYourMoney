...
def test_expense_update_higher_value(new_expense):
    ...
    assert (
        form.errors["cost"][0]
        == "Ensure that there are no more than 5 digits before the decimal point."
    )
...
def test_expense_update_more_decimal_places(new_expense):
    ...
    assert result is False
    assert (
        form.errors["cost"][0] == "Ensure that there are no more than 2 decimal places."
    )
...
def test_income_update_success(new_income):
    form = UpdateIncomeForm(
        1,
        {"amount": "7000", "income_date": "2023-05-05", "source": "1", "currency": "1"},
    )
    result = form.is_valid()
    assert result is True
...
def test_income_update_negative_amount(new_income):
    form = UpdateIncomeForm(
        1,
        {"amount": "-500", "income_date": "2023-05-05", "source": "1", "currency": "1"},
    )
    result = form.is_valid()
    assert result is False
