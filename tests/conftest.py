...
def user_logged_db_populated(
    user_login, new_currency, new_source, new_category, new_income, new_expense
):
    return {
...
def currency_factory(db):
    def create_currency(name: str, currency_code: str = str):
        currency = Currency.objects.create(name=name, currency_code=currency_code)
        return currency
...
def category_factory(db, new_user):
    def create_category(name: str, user=new_user):
