...
def categories_list(request):
    if request.user.is_authenticated:
        return Category.objects.filter(user=None) | Category.objects.filter(
            user=request.user
        )
...
def sources_list(request):
    if request.user.is_authenticated:
        return Source.objects.filter(user=None) | Source.objects.filter(
            user=request.user
        )
...
class ExpenseFilter(django_filters.FilterSet):
    category = django_filters.ModelChoiceFilter(
        label="Kategoria", queryset=categories_list
    )
    cost = django_filters.RangeFilter(label="Wartość")
    expense_date__gte = django_filters.DateFilter(
        widget=DateInput(attrs={"type": "date"}),
        label="Od",
        field_name="expense_date",
        lookup_expr="gte",
    )
    expense_date__lte = django_filters.DateFilter(
        widget=DateInput(attrs={"type": "date"}),
        label="Do",
        field_name="expense_date",
        lookup_expr="lte",
